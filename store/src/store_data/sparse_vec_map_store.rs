...
use vec_map::{SparseVecMap, SparseVecMapKeys};

pub struct SparseVecMapStoreKeys<'a, Key>
where
    Key: Into<usize> + 'static,
{
    storage: Option<Ref<'a, SparseVecMap<Key, TypedData>>>,
}

impl<'a, Key> IntoIterator for &'a SparseVecMapStoreKeys<'a, Key>
where
    Key: Copy + PartialEq + Into<usize> + 'static,
{
    type Item = &'a Key;

    type IntoIter = SparseVecMapKeys<'a, Key>;

    fn into_iter(self) -> Self::IntoIter {
        if let Some(storage) = &self.storage {
            storage.keys()
        } else {
            SparseVecMapKeys(None)
        }
    }
}

pub type SparseVecMapStore<Key> = StoreData<RefCell<SparseVecMap<Key, TypedData>>>;

impl<Key> SparseVecMapStore<Key>
where
    Key: Copy + PartialEq + Into<usize>,
{
...
impl<'a, Key> StoreTrait<'a> for SparseVecMapStore<Key>
where
    Key: Copy + PartialEq + Into<usize> + 'static,
{
    type Key = Key;
    type KeyIterator = SparseVecMapStoreKeys<'a, Key>;
    ...
    fn keys<T>(&self) -> SparseVecMapStoreKeys<Key>
    where
        T: 'static,
    {
        if let Some(storage) = self.get_storage::<T>() {
            SparseVecMapStoreKeys {
                storage: Some(storage.borrow()),
            }
        } else {
            SparseVecMapStoreKeys { storage: None }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn type_keys() {
        let mut sparse_vec_map_store = SparseVecMapStore::<usize>::default();
        sparse_vec_map_store.insert(0, false);
        sparse_vec_map_store.insert(2, true);
        sparse_vec_map_store.insert(4, true);
        sparse_vec_map_store.insert(8, false);

        let keys = sparse_vec_map_store.keys::<bool>();
